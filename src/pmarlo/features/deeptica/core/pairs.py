...
def build_pair_info(
    ...
    *,
    ...
    return PairInfo(idx_t=idx_t, idx_tau=idx_tau, weights=weights_arr, diagnostics=diagnostics)
...
def _normalize_weights(count: int, weights: np.ndarray | None) -> np.ndarray:
    ...
    if arr.size != count:
        raise ValueError("weights must have the same length as the number of lagged pairs")
    return arr
