...
from .markov_state_model.markov_state_model import EnhancedMSM as MarkovStateModel, run_complete_msm_analysis
...
class Pipeline:
    ...
    def __init__(self, 
        ...
        self.protein = None
        self.replica_exchange = None
        self.simulation = None
        self.markov_state_model = None
        
        # Paths
        self.prepared_pdb = None
        ...
        if enable_checkpoints:
            ...
            if not checkpoint_id and auto_continue:
                auto_detected = CheckpointManager.auto_detect_interrupted_run(str(self.output_dir))
                if auto_detected:
                    self.checkpoint_manager = auto_detected
                    logger.info(f"Auto-continuing interrupted run: {auto_detected.run_id}")
                else:
    ...
    def setup_protein(self, ph: float = 7.0) -> Protein:
        ...
        logger.info(f"Protein prepared: {properties['num_atoms']} atoms, {properties['num_residues']} residues")
    ...
    def setup_simulation(self) -> Simulation:
        ...
        self.simulation = Simulation(
            pdb_file=str(self.prepared_pdb),
            temperature=self.temperatures[0],  # Use first temperature for single simulation
            steps=self.steps,
            output_dir=str(sim_output_dir),
            use_metadynamics=self.use_metadynamics
        )
        
        logger.info(f"Simulation setup for {self.steps} steps at {self.temperatures[0]}K")
    ...
    def setup_markov_state_model(self) -> MarkovStateModel:
        ...
        self.markov_state_model = MarkovStateModel(
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.checkpoint_manager or not self.checkpoint_manager.is_step_completed("protein_preparation"):
                if self.checkpoint_manager:
                ...
                if self.checkpoint_manager:
                    self.checkpoint_manager.mark_step_completed("protein_preparation", {
                ...
                self.prepared_pdb = Path(state.get("prepared_pdb", self.output_dir / "prepared_protein.pdb"))
            ...
            if self.use_replica_exchange:
                ...
                if not self.checkpoint_manager or not self.checkpoint_manager.is_step_completed(step_name):
                    if self.checkpoint_manager:
                    ...
                    if self.checkpoint_manager:
                        ...
                        self.checkpoint_manager.mark_step_completed(step_name, {
                            "trajectory_files": trajectory_files
                        })
                else:
                    logger.info("Replica exchange simulation already completed, loading results...")
                    state = self.checkpoint_manager.load_state()
                ...
                if isinstance(trajectory_files, dict) and "demuxed" in trajectory_files:
                    ...
                    self.trajectory_files = list(trajectory_files.values()) if isinstance(trajectory_files, dict) else trajectory_files
                ...
                if not self.checkpoint_manager or not self.checkpoint_manager.is_step_completed(step_name):
                    ...
                    state = self.checkpoint_manager.load_state()
                    trajectory_file = state.get("trajectory_file")
            ...
            if not self.checkpoint_manager or not self.checkpoint_manager.is_step_completed(step_name):
                ...
                for step in self.checkpoint_manager.life_data["completed_steps"]:
                    if step["name"] == step_name:
                        ...
                        break
                msm_results = checkpoint_data.get("analysis_results", {}) if checkpoint_data else {}
            ...
            if self.checkpoint_manager:
                current_stage = self.checkpoint_manager.life_data.get("current_stage", "unknown")
            ...
            results["pipeline"] = {
                ...
                "error": str(e),
                "output_dir": str(self.output_dir),
                "checkpoint_id": self.checkpoint_manager.run_id if self.checkpoint_manager else None
            }
    ...
    def can_continue(self) -> bool:
        ...
        """
        return self.checkpoint_manager and self.checkpoint_manager.can_continue()


# Convenience function for the 5-line API
def run_pmarlo(pdb_file: str, 
               temperatures: List[float] = None,
               steps: int = 1000,
               n_states: int = 50,
               output_dir: str = "pmarlo_output",
...
class LegacyPipeline:
    ...
    def run_legacy_remd_pipeline(self, steps=1000, n_states=50):
        ...
        if self.continue_run and self.run_id:
            ...
            # Start new run
            self.checkpoint_manager = CheckpointManager(self.run_id, str(self.output_base_dir))
            self.checkpoint_manager.setup_run_directory()
        ...
        try:
            ...
            while True:
                ...
                if next_step in [s.get("name") for s in self.checkpoint_manager.life_data["failed_steps"]]:
                ...
                if next_step == "protein_preparation":
                    ...
                    self.checkpoint_manager.mark_step_completed("system_setup", {
                    ...
                    self.checkpoint_manager.mark_step_completed("trajectory_analysis", {
            ...
            print(f"python main.py --mode remd --id {self.checkpoint_manager.run_id} --continue")
            return None