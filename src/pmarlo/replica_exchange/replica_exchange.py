...
class ReplicaExchange:
    ...
    def __init__(self, 
                 ...
                 auto_setup: bool = False):  # Explicit opt-in for auto-setup
        """
        ...
        for i, temperature in enumerate(self.temperatures):
            ...
            try:
                ...
                if abs(energy_val) > 1e6:
                    logger.warning(f"  Very high initial energy ({energy_val:.2e} kJ/mol) detected for replica {i}")
            ...
            for attempt, (max_iter, tolerance_val) in enumerate([
                ...
                try:
                    ...
                    logger.info(f"  Stage 1 minimization completed for replica {i} (attempt {attempt + 1})")
                    minimization_success = True
                    break
                except Exception as e:
                    logger.warning(f"  Stage 1 minimization attempt {attempt + 1} failed for replica {i}: {e}")
                    if attempt == 2:  # Last attempt
                        logger.error(f"  All minimization attempts failed for replica {i}")
                        raise RuntimeError(f"Energy minimization failed for replica {i} after 3 attempts. "
            ...
            if minimization_success:
                try:
                    ...
                    if 'nan' in energy_str or 'inf' in energy_str:
                        raise ValueError(f"Invalid energy ({energy}) detected after minimization for replica {i}")
                    
                    # Check for reasonable energy range
                    energy_val = energy.value_in_unit(unit.kilojoules_per_mole)
                    if abs(energy_val) > 1e5:
                        logger.warning(f"  High final energy ({energy_val:.2e} kJ/mol) for replica {i}")
                    ...
                    if np.any(np.isnan(pos_array)) or np.any(np.isinf(pos_array)):
                        raise ValueError(f"Invalid positions detected after minimization for replica {i}")
                        
                    logger.info(f"  Final energy for replica {i}: {energy}")
                    
                except Exception as e:
                    logger.error(f"  Stage 2 minimization or validation failed for replica {i}: {e}")
                    # Try to recover by using the Stage 1 result
                    logger.warning(f"  Attempting to continue with Stage 1 result for replica {i}")
                    try:
                        state = simulation.context.getState(getEnergy=True)
                        energy = state.getPotentialEnergy()
                        logger.info(f"  Using Stage 1 energy for replica {i}: {energy}")
                    except:
                        raise RuntimeError(f"Complete minimization failure for replica {i}")
    ...
    def is_setup(self) -> bool:
        ...
        """
        return self._is_setup and len(self.contexts) == self.n_replicas and len(self.replicas) == self.n_replicas
    
    def auto_setup_if_needed(self, bias_variables: List = None):
        """
    ...
    def save_checkpoint_state(self) -> Dict[str, Any]:
        ...
        for i, context in enumerate(self.contexts):
            try:
                sim_state = context.getState(getPositions=True, getVelocities=True, getEnergy=True)
                replica_data.append({
                ...
                replica_data.append(None)
        
        state["replica_data"] = replica_data
        return state
    
    def restore_from_checkpoint(self, checkpoint_state: Dict[str, Any], bias_variables: List = None):
        """
        ...
        if not checkpoint_state.get("setup", False):
            logger.info("Checkpoint indicates replicas were not set up, setting up now...")
            self.setup_replicas(bias_variables=bias_variables)
            return
        ...
        self.replica_states = checkpoint_state.get("replica_states", list(range(self.n_replicas)))
        self.state_replicas = checkpoint_state.get("state_replicas", list(range(self.n_replicas)))
        ...
        logger.info(f"Checkpoint restoration complete. Exchange stats: {self.exchanges_accepted}/{self.exchange_attempts}")
    ...
    def calculate_exchange_probability(self, replica_i: int, replica_j: int) -> float:
        ...
        if replica_i < 0 or replica_i >= len(self.contexts):
            raise ValueError(f"replica_i={replica_i} is out of bounds [0, {len(self.contexts)})")
        if replica_j < 0 or replica_j >= len(self.contexts):
            raise ValueError(f"replica_j={replica_j} is out of bounds [0, {len(self.contexts)})")
    ...
    def attempt_exchange(self, replica_i: int, replica_j: int) -> bool:
        ...
        if replica_i < 0 or replica_i >= self.n_replicas:
            raise ValueError(f"replica_i={replica_i} is out of bounds [0, {self.n_replicas})")
        if replica_j < 0 or replica_j >= self.n_replicas:
            raise ValueError(f"replica_j={replica_j} is out of bounds [0, {self.n_replicas})")
        if replica_i >= len(self.contexts):
            raise RuntimeError(f"replica_i={replica_i} >= len(contexts)={len(self.contexts)}")
        if replica_j >= len(self.contexts):
            raise RuntimeError(f"replica_j={replica_j} >= len(contexts)={len(self.contexts)}")
        ...
        if np.random.random() < prob:
            ...
            if replica_i >= len(self.replica_states):
                raise RuntimeError(f"replica_states array too small: {len(self.replica_states)}, need {replica_i + 1}")
            if replica_j >= len(self.replica_states):
                raise RuntimeError(f"replica_states array too small: {len(self.replica_states)}, need {replica_j + 1}")
                
            old_state_i = self.replica_states[replica_i]
            old_state_j = self.replica_states[replica_j]
            
            # Validate state indices
            if old_state_i >= len(self.state_replicas) or old_state_j >= len(self.state_replicas):
                raise RuntimeError(f"Invalid state indices: {old_state_i}, {old_state_j} vs array size {len(self.state_replicas)}")
