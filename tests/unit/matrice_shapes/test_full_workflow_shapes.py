    ...
    br = build_mod.BuildResult(
        ...
        artifacts={
            "mlcv_deeptica": {
                ...
                "top_eigenvalues": history.get("top_eigenvalues", []),
                "whitening": history.get("whitening", {}),
            }
        },
        fes=fes,
    )
...
def test_msm_expansion_to_full_state_space(simple_msm):
    msm_mod = simple_msm["mod"]
    T_active = np.array([[0.7, 0.3], [0.4, 0.6]], dtype=float)
    pi_active = np.array([0.6, 0.4], dtype=float)
    T_full, pi_full = msm_mod._expand_results(4, np.array([0, 2]), T_active, pi_active)
    assert T_full.shape == (4, 4)
    assert pi_full.shape == (4,)
    assert np.allclose(T_full[np.ix_([0, 2], [0, 2])], T_active)
...
def test_build_result_all_matrix_shapes_with_deeptica(
    ...
    assert deeptica_workflow["outputs"].shape[1] == 2


def test_build_result_deeptica_artifacts_shapes(build_result_payload):
    artifacts = build_result_payload["result"].artifacts["mlcv_deeptica"]
    variance = np.asarray(artifacts["output_variance"])
    whitening = artifacts["whitening"]
    assert variance.ndim == 1
    assert variance.shape[0] in {0, 2}
    assert np.asarray(whitening.get("transform", np.eye(2))).shape == (2, 2)
...
def test_json_serialization_preserves_matrix_shapes(build_result_payload):
    result = build_result_payload["result"]
    text = result.to_json()
    loaded = build_mod.BuildResult.from_json(text)
    assert loaded.transition_matrix.shape == result.transition_matrix.shape
    assert loaded.stationary_distribution.shape == (
        result.stationary_distribution.shape
    )
    assert loaded.cluster_populations.shape == result.cluster_populations.shape
    assert loaded.fes.F.shape == result.fes.F.shape


def test_array_payload_serialization_shapes():
    arr = np.arange(12, dtype=np.float32).reshape(3, 4)
    payload = build_mod._serialize_array_payload(arr)
    assert payload["shape"] == [3, 4]
    restored = np.frombuffer(base64.b64decode(payload["data"]), dtype=np.float32)
    assert restored.size == arr.size
