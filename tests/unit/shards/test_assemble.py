...
def _canonical_shard_id(kind: str, *, temperature: float, segment_id: int, replica_id: int) -> str:
...
def _write_shard(tmp_path: Path, *, kind: str, segment_id: int, replica_id: int = 0) -> Path:
    ...
    temperature = 300.0
    shard_id = _canonical_shard_id(kind, temperature=temperature, segment_id=segment_id, replica_id=replica_id)
    meta = ShardMeta(
    ...
    X = np.arange(meta.n_frames * len(feature_spec.columns), dtype=np.float32).reshape(meta.n_frames, len(feature_spec.columns))
    shard = Shard(
        meta=meta,
        X=X,
