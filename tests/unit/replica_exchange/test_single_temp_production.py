...
def test_single_replica_production_runs_md_steps(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            total_production_steps >= production_steps
        ), f"Production should run at least {production_steps} steps, but ran {total_production_steps}"
...
def test_large_exchange_frequency_triggers_single_temp_path():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        is_single_temp = (
            remd.n_replicas == 1
            or (production_steps - equilibration_steps) < remd.exchange_frequency
        )
