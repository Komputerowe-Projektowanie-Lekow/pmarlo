...
def test_dcd_header_format_compliance(test_system):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert header['start_step'] == 100, f"start_step should equal reportInterval (100), got {header['start_step']}"
        assert header['step_interval'] == 100, f"step_interval should be 100, got {header['step_interval']}"
        
        # Timestep should be integration timestep (2 fs = 0.002 ps)
        assert abs(header['timestep'] - 0.002) < 1e-6, f"timestep should be 0.002 ps, got {header['timestep']}"
        
        # Title should have 2 lines of 80 characters each
        assert header['num_title_lines'] == 2, f"Should have 2 title lines, got {header['num_title_lines']}"
        assert len(header['title_data']) == 160, f"Title should be 160 bytes (2x80), got {len(header['title_data'])}"
...
def test_dcd_frame_count_consistency(test_system):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert nframes_header == 15, f"Header should claim 15 frames, got {nframes_header}"
        assert actual_frames >= 15, f"File should contain at least 15 frames, got {actual_frames}"
...
def test_dcd_update_frequency(test_system):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert frame_count == 10, f"Frame count should be 10 after flush, got {frame_count}"
...
def test_dcd_vs_openmm_dcdreporter_compatibility(test_system):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert traj_fast.n_frames == traj_openmm.n_frames, \
...
def test_dcd_crash_recovery_with_frequent_updates(test_system):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert traj.n_frames >= 48, f"Expected at least 48 frames after 'crash', got {traj.n_frames}"
