...
def test_fixed_seed_determinism_two_runs(test_fixed_pdb_file: Path, tmp_path: Path):
    ...
    sim1 = run_short_sim(
        Path(test_fixed_pdb_file), ws, temps, steps=500, quick=True, random_seed=123456
    )
    sim2 = run_short_sim(
        Path(test_fixed_pdb_file), ws, temps, steps=500, quick=True, random_seed=123456
    )
...
def test_auto_seed_varies_trajectories(test_fixed_pdb_file: Path, tmp_path: Path):
    ...
    sim1 = run_short_sim(
        Path(test_fixed_pdb_file), ws, temps, steps=400, quick=True, random_seed=seed1
    )
    sim2 = run_short_sim(
        Path(test_fixed_pdb_file), ws, temps, steps=400, quick=True, random_seed=seed2
    )
