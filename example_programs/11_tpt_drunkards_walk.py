...
def main():
    ...
    output_dir = example_output_dir("11_tpt_drunkards_walk")
    ...
    print(
        f"Home population: {np.sum([msm.stationary_distribution[s] for s in source]):.4f}"
    )
    print(
        f"Bar population: {np.sum([msm.stationary_distribution[s] for s in sink]):.4f}"
    )
