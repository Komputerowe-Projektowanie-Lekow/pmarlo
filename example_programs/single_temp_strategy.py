...
OUTPUT_ROOT = Path(__file__).resolve().parent / "programs_outputs" / "single_temp_strategy"
...
def run_ensemble_strategy(n_runs: int = 5) -> None:
    ...
    print(f"Total frames: ~{len(trajectory_files) * 1000} (assuming 1k frames/trajectory)")
...
def main():
    ...
    print("="*80)
    print("\nYou can now integrate this into your webapp by setting:")
    print("  config.single_temperature_mode = True")
    print("  config.temperatures = [300.0]  # Your target temperature")
    print("\nThe webapp will automatically use run_single_temperature_md()")
    print("instead of run_replica_exchange().")
