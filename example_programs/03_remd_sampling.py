...
Usage:
    python example_programs/03_remd_sampling.py
...
def main():
    ...
    # Find test PDB
    pdb_candidates = [assets_path("3gd8-fixed.pdb"), assets_path("3gd8.pdb")]
    pdb_file = None
    ...
    # Setup
    output_dir = example_output_dir("03_remd_sampling")
    ...
    try:
        ...
        print(
            f"Throughput: {total_steps * len(temperatures) / elapsed:.1f} total steps/sec"
        )
        print()
        ...
        if steps_per_sec > 20:
            ...
            print(
                f"   Throughput ({steps_per_sec:.1f} steps/s) is within expected range."
            )
            print("   Platform selection fix is working correctly!")
        elif steps_per_sec > 10:
            print("WARNING: PERFORMANCE MARGINAL")
            print(
                f"   Throughput ({steps_per_sec:.1f} steps/s) is slower than expected."
            )
