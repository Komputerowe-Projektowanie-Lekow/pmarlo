...
def render_validation_tab(ctx: AppContext) -> None:
    ...
    if not shard_groups:
        ...
        # Data Selection: only show the shard selection UI here. Keep rest of controls collapsed in separate expanders.
        with st.expander("Data Selection", expanded=True):
            ...
            if not selected_runs:
                ...
                return
            profile_summary = summarize_selected_feature_profiles(shard_groups, selected_runs)
            if len(profile_summary["feature_types"]) > 1:
                st.warning(
                    "Selected shard groups mix CV-based and molecular features. "
                    "Validation plots assume a consistent set of variables."
                )
        ...
        # Resolve selected shard paths (stop on error)
        try:
        ...
        # Show a brief caption about selection
        st.caption(f"Using {len(selected_paths)} shard file(s) for validation.")

        # TICA Parameters (collapsed)
        with st.expander("TICA Projection Parameters", expanded=False):
            ...
            val_n_components = col1.number_input(
                ...
                help="Number of TICA components to compute (min 2, max 20).",
            )
            # Allow arbitrarily large integer lag values (no fixed upper bound)
            val_lag = col2.number_input(
                ...
                format="%d",
                key="validation_lag",
                help="Lag time (in steps) for TICA; integer values allowed with no upper bound.",
            )
        ...
        # Clustering Parameters (collapsed)
        ...
        # Sampling Plot Appearance Controls (collapsed)
        with st.expander("Sampling Plot Appearance Controls", expanded=False):
            st.markdown(
        ...
        # FES Plot Controls (collapsed)
        with st.expander("Free Energy Surface (FES) Plot Controls", expanded=False):
            st.markdown("Customize the 2D Free Energy Surface visualization on TICA 1 vs TICA 2.")
            ...
            with col_fes3:
                ...
                cmap_index = available_colormaps.index(default_cmap) if default_cmap in available_colormaps else 0
                st.selectbox(
                    "Colormap",
                    available_colormaps,
                    index=cmap_index,
        ...
        if st.button("Generate Validation Plots", key="run_validation", type="primary"):
            try:
                ...
                with st.spinner("Loading shard data..."):
                    ...
                    run_biased = {}  # Track whether each run is biased
                    ...
                    for shard_path in selected_paths:
                        ...
                        # Check if this shard is biased (from provenance metadata)
                        is_biased = bool(details.source.get("has_bias", False))
                        if run_id not in run_biased:
                            run_biased[run_id] = is_biased
                        elif run_biased[run_id] != is_biased:
                            # If different shards from same run have different bias status,
                            # mark as biased (conservative approach)
                            run_biased[run_id] = True
                    ...
                    for run_id in sorted(run_shards.keys()):
                        ...
                        metabiased_flags.append(run_biased.get(run_id, False))
                    ...
                    # Show which runs are biased
                    biased_runs = [label for label, is_biased in zip(run_labels, metabiased_flags) if is_biased]
                    unbiased_runs = [label for label, is_biased in zip(run_labels, metabiased_flags) if not is_biased]
                    if biased_runs:
                        st.info(f"🎯 Biased/metabiased runs ({len(biased_runs)}): {', '.join(biased_runs)}")
                    if unbiased_runs:
                        st.info(f"📊 Standard runs ({len(unbiased_runs)}): {', '.join(unbiased_runs)}")
                    ...
                    with st.spinner("Generating sampling plot..."):
                        try:
                            sampling_fig = create_sampling_validation_plot(
                                ...
                                metabiased_runs=metabiased_flags,
